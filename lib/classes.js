...
export async function getAllClassIds() {
  const res = await fetch(
    "https://frozen-depths-76264.herokuapp.com/api/classes"
  );
...
export async function getClassData(slug) {
  ...
  const res = await fetch(
    `https://frozen-depths-76264.herokuapp.com/api/classes?${classQuery}`
  );
  const json = await res.json();
  const classData = json.data;

  const labRes = await fetch(
    `https://frozen-depths-76264.herokuapp.com/api/labs?${labQuery}`
  );
